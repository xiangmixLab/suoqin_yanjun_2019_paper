function ms = msGenerateVideoObj_auto(dirName, filePrefix)
...
%   Detailed explanation goes here
    ...
    for i=1:length(aviFiles)
        ...
        if (~isempty(strfind(aviFiles(i).name,filePrefix)))
            try
                if ~isempty(strfind(aviFiles(i).name,'1_'))&&isempty(strfind(filePrefix,'1_'))
                    filePrefix=[filePrefix,'1_'];
                end
                if ~isempty(strfind(aviFiles(i).name,'2_'))&&isempty(strfind(filePrefix,'2_'))
                    filePrefix=[filePrefix,'2_'];
                end
                VidObjtest=VideoReader([dirName '\' filePrefix num2str(count) '.avi']);
                ms.numFiles = max([ms.numFiles str2double(aviFiles(i).name((length(filePrefix)+1):endIndex))]);
                mslist=[mslist, count];
                count=count+1;
            catch
                disp([dirName '\' filePrefix num2str(i) '.avi' ' ' 'corrupted']);
...
%         [folder '\' num2str(filePrefix) num2str(i) '.avi']
        ms.vidObj{i} = VideoReader([dirName '\' filePrefix num2str(i) '.avi']);
...
%     %in folder path
    ...
    unique_camNum=unique(camNum);
    ms.msCam_num=unique_camNum(find(unique_camNum~=ms.camNumber));
end
